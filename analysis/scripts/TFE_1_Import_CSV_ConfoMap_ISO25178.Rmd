...
units_var <- unlist(imp_data[3, data_keep_col])[var_num] #extracts 'unit' line for considered columns
names(units_var) <- head_data_keep[var_num] #gets names associated to the units
units_var_table <- data.frame(variable = names(units_var), unit = units_var) #puts all of it into a data.frame
```
...
colnames(data_final)[1:9] <- c("Sample", "Cycle","Objective", "Location", "Area", "Spot", "Analysis.date", "Analysis.time", "Acquisition.date.time")
```
